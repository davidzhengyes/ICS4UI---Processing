class Row {
  ...
  Row(int numSquares, float y, int dir, float speed, float ps, float f, color c) {
    this.yC = y;
    ...
    squares = new Square[ numSquares + 1 ];   //We actually create 1 extra square, off-screen to the left, 
    ...
    float x = -halfDiagonalLength;                        //The first square is off-screen by 1 halfDiagonalLength
    ...
    for(int i = 0; i < this.squares.length; i++) {
      ...
      if( alternateSpinDirectionsInRow )
        spinDirectionForNextSquare *= -1;
        
      //GIVING EACH SQUARE THE OPPOSITE COLOUR FROM ITS LEFT NEIGHBOUR, IF DESIRED
      if( squareColour == squareCol1 )
        squareColour = squareCol2;
        
      else
        squareColour = squareCol1;
      ...
      x += squareDiagonalLength;
      squarePhaseShift += deltaSquarePhaseShift;
      squareFreq += deltaSquareFreq;
    }
  }
