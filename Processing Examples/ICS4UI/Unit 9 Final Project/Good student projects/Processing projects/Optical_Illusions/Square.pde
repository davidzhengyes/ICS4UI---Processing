class Square {
  ...
  void drawMe() {
    ...
    rotate( (this.freq*d.theta + this.phaseShift)*this.spinDirection );  //The rotate() command rotates the object about the current origin (which is now (xC, yC)!)
                                         //NOT the centre of the object! So to spin the object about its centre, we must create 
                                         //the square at (0, 0) instead of (xC, yC).
                                         
    noStroke();
    rect(0, 0, d.squareWidth, d.squareLength);           //Note that we give (0, 0) as the coordinates, NOT (xC, yC)!
    stroke(0,255,255);
  ...
  void recycleIfOffScreen() {
    ...
    if ( this.myRow.xSpeed > 0 && this.xC >= width + d.halfDiagonalLength ) {  //If we slide off-screen to the right...
      this.xC = -d.halfDiagonalLength;   //...then teleport to the left side, just off-screen
      
      if ( d.oppositeSquareColours && this.myRow.hasOddLength ) //...and possibly switch colours to maintain the alternating-colour pattern.
        this.switchColours();                                 //...Can you see why this is necessary if the row length is odd? :^)
    }
      
    if ( this.myRow.xSpeed < 0 && this.xC <= -d.halfDiagonalLength  ) {       //If we slide off-screen to the left...
      this.xC = width + d.halfDiagonalLength;
      
      if ( d.oppositeSquareColours && this.myRow.hasOddLength )
        this.switchColours();
    }
              
    if( this.yC <= -d.halfDiagonalLength )    
      this.yC = d.screenHeight + d.halfDiagonalLength;     
  }
