...
void setup () {
  ...
  angleIncrement = getReducedFraction( angleIncrement );  //REDUCE THE ORIGINAL FRACTION IF NECESSARY
...
void printTable( int numerator, int denominator ) {
  ...
  text( "x", xAngleColumn, yHeader);
  fill(0,255,0);  //SINE COLUMN IS GREEN
  text( "sin x", xSinColumn, yHeader);
  fill(255,0,0);  //COSINE COLUMN IS RED
  text( "cos x", xCosColumn, yHeader);
  ...
  while( currAngle <= 2*PI + .001 ) 
  {
      ...
      fill(255);
      text( currFrac, xAngleColumn, currY ); //PRINTS THE ANGLE
      fill(0,255,0);
      ...
      currY += rowHeight;  //UPDATES THE Y-VALUE AT WHICH THE ROW WILL BE PRINTED
  }
}
